...
function mergeRecords(current = [], imported = []) {
  const merged = new Map(current.map((record) => [record.key, normalizeAnswerRecord(record)]));
  for (const candidate of imported.map(normalizeAnswerRecord).filter((record) => record.key && record.answer)) {
...
export function mergeAnswerRecords(current = [], imported = []) {
  return mergeRecords(
    upsertAnswerRecords([], current),
    upsertAnswerRecords([], imported),
  );
}
