chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  chrome.storage.local.get({ pendingLearnedAnswers: [] }).then(({ pendingLearnedAnswers }) => {
    ...
    for (const record of message.records) {
      const duplicate = merged.find((item) => item.key === record.key && item.answer === record.answer && item.tabId === tabId);
      if (!duplicate) {
        merged.push({ ...record, tabId, learnedAt: new Date().toISOString() });
        added += 1;
      }
    }
    return chrome.storage.local.set({ pendingLearnedAnswers: merged }).then(() => added);
  }).then((added) => {
    chrome.runtime.sendMessage({ type: 'JOB_AUTOFILL_LEARNED_SAVED', added, tabId }).catch(() => {});
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  chrome.storage.local.get({ pendingLearnedAnswers: [], answerRecords: [] }).then((stored) => {
    const scoped = stored.pendingLearnedAnswers.filter((record) => record.tabId == null || record.tabId === message.tabId);
    const approved = scoped.filter((record) => record.sensitivity === 'safe')
    ...
    const remaining = stored.pendingLearnedAnswers.filter((record) => !(record.tabId == null || record.tabId === message.tabId) || record.sensitivity !== 'safe');
